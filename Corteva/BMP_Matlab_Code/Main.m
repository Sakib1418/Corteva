function [Population,Params] = Main(varargin)
format long g;
...
if ~isempty(varargin)
    ...
    %replaced after the prepareParams() function.
    tmp = readData();

    %Call the prepareParams() function to create a permanent params array and
...
%seed bank
Population.seedBank = zeros(nYears,nGeno,nCohorts);

%Lower seed bank density
Population.lowerBank = zeros(nYears,nGeno);

%immigrant seed
Population.immSeed = zeros(nYears,nGeno,nCohorts);

%emigrant seed
Population.emSeed = zeros(nYears,nGeno,nCohorts);

%germination storage
Population.germination = zeros(nYears,nGeno,nCohorts);

%ungerminated storage
Population.ungerminated = zeros(nYears,nGeno,nCohorts);

%herbicide storage
Population.herbicide1 = zeros(nYears,nGeno,nCohorts);
Population.herbicide2 = zeros(nYears,nGeno,nCohorts);
Population.herbicide3 = zeros(nYears,nGeno,nCohorts);
Population.herbicide4 = zeros(nYears,nGeno,nCohorts);

%adult plant storage
Population.mature = zeros(nYears,nGeno,nCohorts);

%hand removal storage
Population.hand = zeros(nYears,nGeno,nCohorts);

%immigrant pollen
Population.immPol = zeros(nYears,nGeno,nCohorts);

%emigrant seed
Population.emPol = zeros(nYears,nGeno,nCohorts);

%seed produced storage
Population.newSeed = zeros(nYears,nGeno,nCohorts);

%seed population after mutation
Population.mutated = zeros(nYears,nGeno,nCohorts);

%seed population after predation
Population.predation = zeros(nYears,nGeno,nCohorts);

%seed population after winter (note this happens after all cohorts
%have done their thing, so cohort data isn't stored)
Population.winter = zeros(nYears,nGeno);

%Resistance metrics data
Population.Res = zeros(1, 3);
...
for y = 1:nYears
    ...
    for c = 1:nCohorts
        ...
        %B. Call transition function

        %survival function with germination parameters (storing
        ...
        %call selective survival function for herbicide1
        ...
        %call selective survival function for herbicide2
        ...
        %call selective survival function for herbicide3
        ...
        %call selective survival function for herbicide4
        ...
        %call competition function for herbicide4
        mature = competition(herbicide4,A,B,C);

        %store curPop value in population structure 
        mature = extinction(mature,fieldSize);

        %% Hand Removal (survival function)
        ...
        %% Mutation
        %Called for loci 1-4
        %Has a bypass built in to avoid repetitive function calls if
...
%                                 Params.General.completeResTime);
    ...
    if y == nYears
        ...
        %call winter survival function for upper seed bank
