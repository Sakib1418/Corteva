function [int_risk,res_risk,res_data,RoverT] = resistanceRisk(nSim)
...
%between years). The function returns the population for each of nSim
...
for n = 1:nSim
    ...
    res_pop = sumResistant(Population.(lifestage)(:,:,end),Params.key);
    
    %total population
    tot_pop = sum(Population.(lifestage)(:,:,end),2);
    ...
    %Find the resistance risk as a function of time
