...
yearEval = np.zeros(nSim,length(fEval))
...
for n = 1:nSim
    ...
    #total after N years, (defined by nYears in defineParams() def)
    R = sumResistant(Population.(lifestage)(:,:,),Params.key) #resistant population
    T = sum(Population.(lifestage)(:,:,),2)    #total population (at  of year)
    ...
    if n==1
        ResistanceRisk = np.zeros(1,Params.General.nYears)
    

    #count the resistants in each year and divide by total population
    resPop = sumResistant(Population.(lifestage)(:,:,),Params.key)
    totPop = sum(Population.(lifestage)(:,:,),2)
...
#Fit a sigmoid to the output (using subdef sigmoidFun, below)
...
subplot(2,2,2)
hold all
# set(0,'DefaultAxesColorOrder',[0 0 11 0 00 0.4 01 0.6 0])
...
def nEval = sigmoidMetrics(n,f,fEval)
    
    #Number of evaluation points
    len = length(fEval)

    #Array to store output
    nEval = np.zeros(1,len)
