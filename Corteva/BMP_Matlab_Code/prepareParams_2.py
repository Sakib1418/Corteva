...
#the def (not most computationally efficient, but improves
#readability)
nLoci = Params.General.nLoci

#Generate a key for relating each genotype to the number of resistant
#alleles it posseses (0,1, or 2) at each gene locus. (Subdef contained
#in this file).
...
## Subdefs
def [key,key_text] = generateKey(nLoci)
...
#   loci 1 and loci 2.
#This def also creates a cell array which contains the same
...
#populate the key array using the def odometer
...
def odo = odometer(odo,base_num)
## Odometer def
#This def counts using a user specified number base. A digit takes
#on integer values between 0 and base_num. This def increments the
...
#[0,0,2]. A second def call on odo will produce odo = [0,1,0].
...
def populationFraction_outCross = matingEquilibrium(p,selfCoeff,key)
q = 1-p
nLoci = size(key,1)
nGeno = size(key,2)
